...
import ar.com.pmp.subastados.SubastadosApp;
...
import ar.com.pmp.subastados.service.mapper.UserMapper;
import ar.com.pmp.subastados.web.rest.errors.ExceptionTranslator;
...
@SpringBootTest(classes = SubastadosApp.class)
public class UserResourceIntTest {
	...
	public void setup() {
		...
		UserResource userResource = new UserResource(userRepository, userService, mailService, persistentTokenRepository);
		this.restUserMockMvc = MockMvcBuilders.standaloneSetup(userResource).setCustomArgumentResolvers(pageableArgumentResolver).setControllerAdvice(exceptionTranslator).setMessageConverters(jacksonMessageConverter).build();
	}
	...
	public void getUser() throws Exception {
		...
		// Get the user
		restUserMockMvc.perform(get("/api/users/{login}", user.getLogin())).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)).andExpect(jsonPath("$.login").value(user.getLogin())).andExpect(jsonPath("$.firstName").value(DEFAULT_FIRSTNAME)).andExpect(jsonPath("$.lastName").value(DEFAULT_LASTNAME)).andExpect(jsonPath("$.email").value(DEFAULT_EMAIL)).andExpect(jsonPath("$.imageUrl").value(DEFAULT_IMAGEURL)).andExpect(jsonPath("$.langKey").value(DEFAULT_LANGKEY));
